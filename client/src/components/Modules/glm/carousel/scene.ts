...
export default class extends ThreeScene {
    ...
    viewer = new Viewer({
        onClose: () => { this.play = true; this.render(); },
    ...
    constructor(container: HTMLElement) {
        ...
        this.radius = Math.max(this.config.minRadius, radiusFromLength(this.images.length));
    ...
    init() {
        ...
        super.init();
        if (!this.images) { return console.warn('No images has been found'); }
        ...
        const { width, height } = normalizeDimension(this.IMAGE_NORM_WIDTH, this.config.pictureWidth, this.config.pictureHeight);
    ...
    events() {
        ...
        const bindedMouseMove = (e: MouseEvent | TouchEvent) => this.rotateObject(e, this.pivot, {
            damping: 0.02,
            inverted: this.config.invertDrag == 1 || this.config.curve == 'inward' && this.config.invertDrag == 0,
    ...
    onMouseMove(e: MouseEvent) {
        ...
        if (raycast.length > 0) {
            ...
            const hoverPlane = raycast.find(intersect => intersect.object instanceof THREE.Mesh);
            if (hoverPlane) {
                this.lastHoverPicture = getPictureFromUUID(hoverPlane.object.uuid, this.images as Picture[]);
            }
        } else {
    ...
    onCanvasClick() {
        ...
        if (this.lastMouseMovement !== 0) { return; }
    ...
    render() {
        ...
        if (this.lastMouseMovement === 0) {
            this.pivot.rotation[this.rotationAxis] += (this.config.curve === "outward" && this.config.direction == "right" || this.config.curve == "inward" && this.config.direction == "left" ? -1 : 1) * this.cameraRotateSpeed / 60;
        }
    }
